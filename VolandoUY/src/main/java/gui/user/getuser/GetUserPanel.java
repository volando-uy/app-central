...
public class GetUserPanel extends JPanel {
    ...
    private void headerClienteSet(String n, String nick, ImageIcon i, String e, String d) {
        name.setText(evadeNullString(n));
        nickname.setText(nick);
        image.setIcon(i);
        email.setText(evadeNullString(e));
        doc.setText(evadeNullString(d));
        showHeaderCard(CARD_CLIENT);
    }

    private void headerAirlineSet(String n, String nick,ImageIcon i, String e, String w, String d) {
        aName.setText(evadeNullString(n));
        aNickname.setText(nick);
        aImage.setIcon(i);
        aEmail.setText(evadeNullString(e));
        ...
        try {
            if ("Todos".equalsIgnoreCase(filtro) || "Usuario".equalsIgnoreCase(filtro)) {
                ...
                for (BaseCustomerDTO c : customers) {
                    String nick = evadeNullString(c.getNickname());
                    if (nick.isBlank()) continue;
                    String nombre = (evadeNullString(c.getName()) + " " + evadeNullString(c.getSurname())).trim();
            ...
            if ("Todos".equalsIgnoreCase(filtro) || "Aerolínea".equalsIgnoreCase(filtro)) {
                ...
                for (BaseAirlineDTO a : airlines) {
                    String nick = evadeNullString(a.getNickname());
                    if (nick.isBlank()) continue;
                    modelUsuarios.addRow(new Object[]{ "Aerolínea", evadeNullString(a.getName()), nick });
                }
            }
    ...
    private void onUsuarioSeleccionado() {
        ...
        try {
            if (usuarioSeleccionadoTipo.equalsIgnoreCase("Aerolínea")) {
                ...
                String nombre   = (evadeNullString(c.getName()) + " " + evadeNullString(c.getSurname())).trim();
                String docTxt   = (enumToString(c.getDocType()) + " " + evadeNullString(c.getNumDoc())).trim();
                String emailTxt = evadeNullString(c.getMail());
    ...
    private void loadAirlineRoutes(String airlineNickname) {
        ...
        try {
            ...
            for (FlightRouteDTO r : rutas) {
                ...
                modelRutas.addRow(new Object[]{ evadeNullString(r.getName()), origin, dest, status });
            }
                ...
                flights = reserva.getTicketIds().stream()
                        .map(ticketId -> {
                            ...
                            return (s != null && s.getFlightName() != null)
                                    ? evadeNullString(s.getFlightName()) : "N/A";
                        })
                        .distinct()
    ...
    private static String evadeNullString(String s) { return s == null ? "" : s; }
