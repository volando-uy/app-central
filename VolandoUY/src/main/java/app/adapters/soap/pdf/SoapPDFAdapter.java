...
public class SoapPDFAdapter extends BaseSoapAdapter {
    ...
    ISeatController seatController = ControllerFactory.getSeatController();
    ...
    public String getBookflightPDFBase64(@WebParam(name = "bookingId") Long bookingId, @WebParam(name = "token") String token) {
        try {
            ...
            // Obtener vuelo (asumiendo que al menos hay un ticket)
            TicketDTO firstTicket = ticketController.getTicketDetailsById(dto.getTicketIds().get(0));
            SeatDTO seat = seatController.getSeatDetailsById(firstTicket.getSeatId());
            FlightDTO flightDTO = flightController.getFlightDetailsByName(seat.getFlightName());
            ...
            document.add(new Paragraph("Fecha de compra: " + dto.getCreatedAt().toLocalDate(), bodyFont));
                ...
                for (Long tid : dto.getTicketIds()) {
                    ...
                    if (ticket.getBasicLuggages() != null && !ticket.getBasicLuggages().isEmpty()) {
                        for (BasicLuggageDTO basic : ticket.getBasicLuggages()) {
                            table.addCell(firstRow ? String.valueOf(ticket.getId()) : "");
                            table.addCell(firstRow ? pasajero : "");
                            table.addCell(firstRow ? doc : "");
                            table.addCell(firstRow ? asiento : "");
